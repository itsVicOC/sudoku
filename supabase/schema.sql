...
begin
  ...
  on conflict (difficulty, puzzle_key, player_id_key)
  do update set
...
as $$
  select
    row_number() over (order by time_ms asc, achieved_at asc) as rank,
    player_id,
    time_ms,
    achieved_at as created_at
  from public.leaderboard_scores
  where difficulty = difficulty_input
    and puzzle_key = puzzle_key_input
